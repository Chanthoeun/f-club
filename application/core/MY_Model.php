...
class MY_Model extends CI_Model
{
    ...
    function dropdown()
    {
        ...
        if(count($args) == 3){
            list($key, $value, $option_label) = $args;
        ...
        $result = $this->_database->select(array($key, $value))
                           ->get($this->_table)
                           ->result();
        empty($option_label) ? $options = array() : $options = array('' => $option_label);
    ...
    public function get_next_order()
    {
        ...
        $this->_database->select_max($field, 'max');
        if($where != FALSE)
        {
            $this->_database->where($where);
        }
        return (int) $this->_database->get($this->_table)->row()->max + 1;
    }
